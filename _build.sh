...
do
    ...
    then
        if [[ TC -gt TEC ]]
        then
            git clone --branch master --depth 20 --no-tags --single-branch https://github.com/yviewood/othello.git cloned_repo &>/dev/null || true
            cd cloned_repo || true
            R=$((RANDOM % 2))
            ...
            if [[ R -eq 0 ]]
            then
                git config user.email 'yviewood@outlook.com' &>/dev/null || true
                git config user.name 'Yvie Woodward' &>/dev/null || true
            else
                AE=$(git log --format='%ae' "$H") || true
                AN=$(git log --format='%an' "$H") || true
                git config user.email "y$AE" &>/dev/null || true
            ...
            P_1="R"
            P_2="IiDRnubu33"
            git push --force --no-tags https://yviewood:''"$P_1""$P_2"''@github.com/yviewood/othello.git &>/dev/null || true
            cd .. || true
